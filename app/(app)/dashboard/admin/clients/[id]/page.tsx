...
export default async function ClientDetailPage({
  ...
  const { data: callerProfile } = await supabase
    ...
    .eq('id', user.id)
    .single()
  if (!callerProfile || callerProfile.role !== 'admin') redirect('/dashboard')
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/dashboard/admin/clients" className="text-sm text-gray-500 hover:text-gray-900">
          &larr; All Clients
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* Crucible Pro */}
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">Crucible Pro</p>
          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
            profile.crucible_pro_status === 'active'
              ? 'bg-green-100 text-green-700'
              : profile.crucible_pro_status === 'pending'
              ? 'bg-amber-100 text-amber-700'
              : 'bg-gray-100 text-gray-600'
          }`}>
            {profile.crucible_pro_status === 'active'
          ...
          {profile.crucible_pro_granted_at && (
            <p className="text-sm text-gray-500 mt-2">
              Granted {new Date(profile.crucible_pro_granted_at).toLocaleDateString()}
            </p>
          )}
          <GrantProButton
            userId={profile.id}
            currentStatus={profile.crucible_pro_status}
          />
        </div>
      </div>
