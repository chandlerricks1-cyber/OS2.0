...
import {
  createPovProPipeline,
  isGhlConfigured,
  listPipelines,
  POV_PRO_PIPELINE_NAME,
  POV_PRO_STAGE_NAMES,
} from '@/lib/ghl/client'
...
export async function GET() {
  ...
  try {
    ...
    const povPro = pipelines.find((p) => p.name.trim().toLowerCase() === POV_PRO_PIPELINE_NAME.toLowerCase())
    return NextResponse.json({
      configured: true,
      pipelineExists: !!povPro,
      povPro: povPro ?? null,
...
export async function POST() {
  ...
  try {
    ...
    const already = existing.find((p) => p.name.trim().toLowerCase() === POV_PRO_PIPELINE_NAME.toLowerCase())
    if (already) {
      return NextResponse.json({
        created: false,
        message: 'POV Pro pipeline already exists',
        pipeline: already,
      })
    }

    const created = await createPovProPipeline()
    ...
    return NextResponse.json(
      {
        ...
        manualInstructions: {
          ...
          pipelineName: POV_PRO_PIPELINE_NAME,
          stages: POV_PRO_STAGE_NAMES,
        },
      },
      { status: 500 }
    )
